...
while($file= mysqli_fetch_array($consulta)){
  ...
  $id_company_auditada = $file['id_company_auditada'];
  $fecha = $file['fecha'];
  $estado = $file['estado'];
  $recommendations = $file['recommendations'];
  $conclusiones = $file['conclusiones'];

}

//sacamos da de la company auditada
$sentencia_company_auditada = "select * from companies where id=".$id_company_auditada; 
...
<div class="contenedor">
  <div class=WordSection1>
    ...
    <span style="font-family:'Verdana',sans-serif; font-size: 12px;">
      Copyright &copy; <?php echo $anioActual; ?> blackstone5 Project. All rights reserved.
    </span>
    </center>
  ...
  <p style='margin-top:0cm;margin-right:23.05pt; margin-bottom:0cm;margin-left:14.2pt;margin-bottom:.0001pt;text-align:center'>
      <?php echo lang("YOU CAN MAKE REPORTS ON AUDITS AND CERTIFICATION EXAMS WITH THE FREE REPORTING TOOL FOR KALI AND PARROT");?> "<b>blackstone5</b>"
      <br><br>
  </p>
  ...
  <p style='margin-top:0cm;margin-right:23.05pt; margin-bottom:0cm;margin-left:14.2pt;margin-bottom:.0001pt;text-align:center'>
      <a href="https://github.com/micro-joan/blackstone5" style="color:blue;">https://github.com/micro-joan/blackstone5</a>
      <br><br>
  </p>
  ...
  <div class=WordSection3>
    ...
    <table>
      ...
      <tr style='height:35.0pt'>
        ...
        <td width=523 valign=top style='width:392.2pt;border:solid black 1.0pt;border-left:none;padding:0cm 5.4pt 0cm 5.4pt;height:35.0pt'>
          ...
          <p class=TableParagraph style='margin-top:.05pt;margin-right:23.05pt;margin-bottom:0cm;margin-left:14.2pt;margin-bottom:.0001pt;text-align:justify'>
            <b>
              <span style='font-size:10.0pt;font-family:"Verdana",sans-serif'>blackstone5<span style='letter-spacing:-.05pt'> 
            </b>
          </p>
        </td>
      </tr>
    ...
    <p>
        ...
        <?php echo lang("and their systems.");?> blackstone5  <?php echo lang("recommends that special precautions be taken to");?>
    ...
    <p>
      ...
      <?php echo lang("or maintained by other providers. blackstone5 bases these recommendations on of your previous experience with the capabilities of these products. However, blackstone5 cannot and should not guarantee that any particular product will perform as advertised by the seller.");?>
    </p>
</div>
...
<table>
  ...
  <?php 
        ...
        $consulta_objetivos_informe = mysqli_query($connection, $cantidad_objetivos_informe) or die("Error de conexión");
        
        $contador_scope = 1;

        while($file_scope = mysqli_fetch_array($consulta_objetivos_informe)){
  ...
  error_reporting(E_ALL);
      ...
      while($file_scope = mysqli_fetch_array($consulta_objetivos_informe)){
          ...
          while($file_vulns_scope = mysqli_fetch_array($consulta_vulns_scope)){
            ...
            $nivel_scope=$file_vulns_scope['nivel'];
...
<?php 
      ...
      while($file_scope = mysqli_fetch_array($consulta_objetivos_informe)){
          ...
          while($file_vulns_scope = mysqli_fetch_array($consulta_vulns_scope)){
            ...
            $nivel_scope0=$file_vulns_scope['nivel'];
            $nivel_scope=$file_vulns_scope['nivel'];
            $name_scope=$file_vulns_scope['name'];
            $descripcion_scope=$file_vulns_scope['descripcion'];
            $recommendation_scope=$file_vulns_scope['solucion'];
            ...
            while($file_vulns_imagen = mysqli_fetch_array($consulta_vulns_imagen)){
              $scope_imagen=$file_vulns_imagen['ruta'];
              $descripcion_imagen=$file_vulns_imagen['descripcion'];
