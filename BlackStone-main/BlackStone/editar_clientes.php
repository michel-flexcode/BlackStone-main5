      ...
      <div class="container-fluid page-body-wrapper">
        ...
        <div class="main-panel">
          ...
          <?php
              ...
              $sentencia = "select * from companies where id=".$id_url;    
              ...
              while($file= mysqli_fetch_array($consulta)){
                $id=$file['id'];
                $name=htmlspecialchars($file['name'], ENT_QUOTES | ENT_HTML5, 'UTF-8');
                $web=htmlspecialchars($file['web'], ENT_QUOTES | ENT_HTML5, 'UTF-8');
                $logo=$file['logo'];
                $logo_last=$file['logo'];
            ...
            <div class="page-header">
              <?php echo $wich; ?>
              <h3 class="page-title"> <?php echo lang("Edit company"); echo " '".$name."'"?> </h3>
            ...
            if (isset($_POST['submit'])){
              ...
              if($logo_last > '' && $name_img > ''){
                unlink($logo_last);
              }

              $directorio ='logos_clientes/';
              move_uploaded_file($_FILES['imagen']['tmp_name'],$directorio.$name_img);

              $logo = $directorio.$name_img;

              //en el caso de que no se inserte una foto nueva que no se altere el parametro en la BBBDD
              if($name_img == "" || $name_img == "null"){

              $sentencia = "UPDATE `companies` SET `name`='$name',`web`='$web' WHERE id=".$id_url.";";
              $consulta = mysqli_query($connection, $sentencia)or die("Error de consulta");

              }else{
                $sentencia = "UPDATE `companies` SET `name`='$name',`web`='$web', `logo`='$logo' WHERE id=".$id_url.";";
                $consulta = mysqli_query($connection, $sentencia)or die("Error de consulta");
              }
